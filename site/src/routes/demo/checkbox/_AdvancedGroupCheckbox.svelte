<script lang="ts">
  import { Checkbox, CheckboxGroup } from "@smui/checkbox";
  ...
  function handleClick(event: CustomEvent<Event & {target: HTMLInputElement}>, index: number) {
    const option = options[index];
    option.selected = event.detail.target.indeterminate ? null : event.detail.target.checked;
