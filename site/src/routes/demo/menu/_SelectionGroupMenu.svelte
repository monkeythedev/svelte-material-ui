<script lang="ts">
  ...
  import { MenuButton } from "@smui/button";

  let menu: Menu;
  let selected1: string = "Red";
  let selected2: string = "Small";
...
<div>
  ...
  <div style="min-width: 100px;">
    <MenuButton {menu} on:click={() => menu.setOpen(true)}>Open Menu</MenuButton>
    <Menu bind:this={menu}>
      <List>
        <SelectionGroup>
          {#each ['Red', 'Green', 'Blue'] as item}
            <Item
              on:click={() => (selected1 = item, clicked = item)}
        ...
        <SelectionGroup>
          {#each ['Small', 'Medium', 'Large'] as item}
            <Item
              on:click={() => (selected2 = item, clicked = item)}
